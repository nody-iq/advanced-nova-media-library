...
export default {
  ...
  methods: {
    ...
    onSave() {
      // Retrieve the result from the cropper component and extract the canvas
      const { canvas } = this.$refs.clipper.getResult();
      // Convert the canvas content to a base64 encoded string based on the image MIME type
      const base64 = canvas.toDataURL(this.mime);
      // Convert the base64 string to a file object using a custom converter function
      const file = Converter(base64, this.mime, this.image.file_name);

      // Prepare the file data object to be emitted, including URLs and file names
      let fileData = {
        file,
        __media_urls__: {
          __original__: base64, // URL for the original cropped image
          default: base64, // Default URL, can be used for thumbnails or other versions
        },
        name: file.name, // Name of the file
        file_name: file.name, // File name stored
      };

      // Emit an event with the cropped file data for further handling
      this.$emit("crop-completed", fileData);
      // Emit an event to signal that the cropping process should close
      this.$emit("close");
    },
